...
# Here, the functional notation from the k-means core loop body equations is used.
...
def get_components(changes):
    """ Extract components from list of changes generated by filling in for k = n+1, n+2, ... """
    components = {}
    for c in changes:
        args = extract_args(c)
...
def gather_sum(component_key, values):
    """ Tries to gather sum components from provided values.
        Returns gathered sum if able. """
    if len(values) == 1:
        return values[0]
    elif len(values) > 1:
        summable = True
...
def create_sum(origin, base, k, step_k, components):
    """ Tries to create a sum using the provided parameters. """
...
def simplify_to_sum(expr, x, n, start: int, end: int = None):
    """ Tries to simplify x from provdided expression to sum. """
    result = expr
    k: int = start
